...
{
	//Use to check for nested Json key for example -> "test": {"test1": 10, "test2": {"test3": 15}} -> this is 1 nest level
...
std::vector<std::string> Json::convertTypeArray(std::string original_string) 
{
	...
	for (const char& character : original_string) {
		switch (character) {
			...
			case ' ':
			case ',':
				if (temp_str[0] == '\"' && temp_str[temp_str.size() - 1] != '\"') {
					temp_str += character;
					break;
				}
				if (!temp_str.empty()) {
			...
			try {
				...
				case ',':
					if (!temp_str.empty()) { //Merge !temp_str.emty()
						if ((temp_str[0] == '\"' && temp_str[temp_str.size() - 1] == '\"') || (temp_str[0] == '[' && temp_str[temp_str.size() - 1] == ']') || (temp_str[0] != '\"' && temp_str[0] != '[')) { 
							//Check if the temp_str is a string and is the program reach the end of the temp_str (temp_str[0] == '\"' && temp_str[temp_str.size() - 1] == '\"') and check if the temp_str is an array and does it end yet (temp_str[0] == '[' && temp_str[temp_str.size() - 1] == ']') and check if the temp_str an int but also not an array because it wont have the "\"" at the beginning or the '['at the beginning (temp_str[0] != '\"' && temp_str[0] != '\""') 
							Json::praser_character_result.push_back(temp_str);
							temp_str = "";
						} else {
...
std::string Json::call(std::vector<std::string> path_to_value) {
	...
	for (const std::string& path : path_to_value) {
...
void Json::change(std::vector<std::string> path_to_value, std::string change_to, char type, bool write_to_file, bool format_when_wrtie)
{
	Json::call(path_to_value);
...
void Json::add(std::vector<std::string> path_to_value, std::string key_name ,std::string value_to_add, char type)
{
	//Add a new key into the json file
	path_to_value.push_back(key_name); //Push back the key name to make it a complete path
	Json::program_ask_for_call = true; //Tell the program that the program is making the call request not the user
	Json::call(path_to_value); //Find the key outer using the path

	/*Error checking*/
	if (Json::where_call_func_find_outer_key_to_value == -1 && path_to_value.size() > 1 && type != 'n') {
		...
		for (const std::string path_to_element : path_to_value) {
			std::cout << path_to_element << " ";
		}
		std::cout << "" << std::endl;
		return;
	} else if (Json::call_func_inner_key_already_exsist || (Json::where_call_func_find_outer_key_to_value != -1 && path_to_value.size() == 1)) {
...
int Json::convertTypeInt(std::string original_value)
{
	return std::stoi(original_value);
}

std::vector<int> Json::convertTypeArrayInt(std::string original_string) 
{
	std::vector<std::string> prase_to_array;
	std::vector<int> result;

	prase_to_array = Json::convertTypeArray(original_string); //Prase the original_value that is a string to a vector that contain string
	
	for (int i = 0; i < prase_to_array.size(); i++) {
		result.push_back(std::stoi(prase_to_array[i])); //Tranfrom each element in the vector so it contain int instead
	}

	return result;
}

std::vector<std::string> Json::convertTypeArrayString(std::string original_string)
{
	std::vector<std::string> prase_to_array;

	prase_to_array = Json::convertTypeArray(original_string); //Prase the original_value that is a string to a vector that contain a string but with double quote

	for (int i = 0; i < prase_to_array.size(); i++) {
		//Remove double quote
		prase_to_array[i].erase(0, 1);
		prase_to_array[i].erase(prase_to_array[i].size() - 1, 1);
	}

	return prase_to_array;
}

bool Json::is_true(std::vector<std::string> path_to_value)
{
	std::string call_result;

	call_result = Json::call(path_to_value);
...
std::map<std::string, std::string> Json::items()
{
	std::map<std::string, std::string> result;
	int this_position;
	int third_position;

	for (int i = 0; i < praser_token_result.size(); i++) {
		this_position = i;
		third_position = i + 2;

		if (praser_token_result[this_position] == KEY) {
			//If the program find a KEY the program will map the KEY to the VALUE {KEY : VALUE}
			result[praser_character_result[this_position]] = praser_character_result[third_position];
		}
	}
...
{	
	...
	for (int i = 0; i < Json::praser_token_result.size(); i++) {
		...
		if (Json::praser_token_result[this_position] == KEY) {
			result.push_back(Json::praser_character_result[this_position]);
		}	
	}
...
std::vector<std::string> Json::values()
{
	std::vector<std::string> result;
	int this_position;
