...
class Json {
	...
	public:
		...
		/*
		The writeJson function is use to write the json data back to the file it take in format which is a bool if it is true
		the file will be format when writing if it is not true the file wont be format
		*/
		...
		*/
		...
		/*
		The convertStringToJson function is use to convert from a string to a Json object this mean you can convert string for example ->
		...
		*/
		...
		template<typename T> 
		T convertType(std::string original_value, char type)
		{
			std::vector<T> array_result;

			switch (type) {
				case 'i':
					return std::stoi(original_value);
				case 'a':
					for (const char& character : original_value) {
						if (character != ',' && character != '[' && character != ']') {
							array_result.push_back(character);
						}
					}
					return array_result;
			}	
	...
	private:
		...
		int nestLevelCheck(int nest_level, int this_position);
